...
ArraySequence<T>::ArraySequence(T *pData, size_t size)
    : array_(new DynamicArray<T>(pData, size)) {}
...
template<typename T>
ArraySequence<T>::~ArraySequence<T>(){
    delete array_;
    array_ = nullptr;
}
...
template<typename T>
void ArraySequence<T>::EraseAt(size_t i) { GetArray()->EraseAt(i); }

template<typename T>
void ArraySequence<T>::PopBack() { GetArray()->PopBack(); }

template<typename T>
ArraySequence<T> * ArraySequence<T>::Concatenation(ISequence<T> *pArray) { return GetArray()->Concatenation(pArray);}
